...
export function Flight3D() {
  ...
  useEffect(() => {
    ...
    let rafId: number;
    const animate = () => {
      if (isVisible && needsRender && renderer && scene && camera) {
    ...
    const manager = new THREE.LoadingManager(() => {
      ...
      myPlane.traverse((child) => {
        if ((child as THREE.Mesh).isMesh) {
          ...
          mesh.frustumCulled = true; // RE-ENABLED: Standard optimization to skip off-screen geometry.
          if (mesh.material) {
      ...
      // ─── BACKGROUND IMAGE PRE-DECODING ───
      const bgAssets = ["/assets/background-reduced.jpg", "/assets/clouds.png", "/assets/cloud-ceiling.png"];
      ...
      const totalFlightDuration = 6;
      if (grounds.length) {
        flightTimeline.to(grounds, { y: "30%", ease: "none", duration: totalFlightDuration }, 0);
      }
      if (deepClouds.length) {
        flightTimeline.from(deepClouds, { y: "20%", ease: "none", duration: totalFlightDuration }, 0);
      }
      if (foregroundClouds.length) {
        flightTimeline.from(foregroundClouds, { y: "25%", ease: "none", duration: totalFlightDuration }, 0);
      }
      ...
      flightTimeline.to(fadeObj, {
        opacity: 1,
        duration: sectionDuration * 0.5,
        ease: "none",
        onUpdate: () => {
          materials.forEach(mat => { mat.opacity = fadeObj.opacity; });
        },
        onComplete: () => {
          materials.forEach(mat => { mat.transparent = false; });
        }
      }, delay);
