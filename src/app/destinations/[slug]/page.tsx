...
export default function DestinationDetailPage({
  ...
  // Unwrap the Promise per Next.js 15
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pkgRes = await fetch(`/api/packages?destination=${slug}`);
        if (pkgRes.ok) {
          const pkgData = await pkgRes.json();
          setPackages(pkgData);
        }

        // 3. Fetch Related Destinations (same region, different slug)
  ...
  const filteredPackages = useMemo(() => {
    ...
    if (filters.duration) {
      result = result.filter(pkg => {
        ...
        const days = nights + 1; // Approximate
    ...
    if (filters.budget) {
      result = result.filter(pkg => {
        const price = parseInt(String(pkg.price).replace(/[^0-9]/g, "")) || 0;
        if (filters.budget === "0-20000") return price <= 20000;
        if (filters.budget === "20000-50000") return price > 20000 && price <= 50000;
        if (filters.budget === "50000-100000") return price > 50000 && price <= 100000;
        if (filters.budget === "100000+") return price > 100000;
        return true;
      });
    }
    ...
    // 3. Trip Type Filter
    if (filters.tripType) {
      result = result.filter(pkg => (pkg as any).trip_type === filters.tripType);
    }

    // 4. Difficulty Filter
    if (filters.difficulty) {
      result = result.filter(pkg => (pkg as any).difficulty_level === filters.difficulty);
    }
    ...
    <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black">
      ...
      <section className="relative w-full h-[60vh] md:h-[70vh] flex items-end pb-16 md:pb-24">
        ...
        <div className="relative z-10 w-full max-w-[1200px] mx-auto px-6">
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-semibold tracking-tight leading-none text-white mb-6">
            {destination.name}
          </h1>

          <div className="flex items-center gap-4 flex-wrap">
            <div className="px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/10 text-[10px] font-bold uppercase tracking-[0.15em] text-white/80">
              {packages.length} Journeys
            </div>
            {destination.region && (
              <div className="px-4 py-2 rounded-full bg-white/5 backdrop-blur-md border border-white/5 text-[10px] font-bold uppercase tracking-[0.15em] text-white/60">
                {destination.region}
              </div>
            )}
          </div>
      ...
      {/* ── FILTER BAR & PACKAGE GRID ── */}
      <section className="px-6 py-16 md:py-24">
        <div className="max-w-[1200px] mx-auto">
          {/* Sticky Filter Bar container */}
          <div className="sticky top-[100px] z-30 bg-black/80 backdrop-blur-2xl py-4 mb-10 border-b border-white/[0.04] -mx-6 px-6">
            <FilterBar 
              filters={filters} 
              onChange={setFilters} 
          ...
          {filteredPackages.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {filteredPackages.map((pkg) => (
                <PackageCard 
                  key={pkg.id} 
                  pkg={pkg} 
                  onClick={(p) => openModal('PACKAGE', p, `DESTINATION_${destination.slug.toUpperCase()}`)} 
                />
              ))}
            </div>
          ) : (
            <div className="py-24 text-center">
              ...
              <p className="text-white/40 text-sm">Try adjusting your filters to see more results.</p>
              <button 
                onClick={() => setFilters({ duration: "", budget: "", tripType: "", difficulty: "", sort: "" })}
                className="mt-6 px-6 py-2.5 rounded-full bg-white/10 text-white text-xs font-bold uppercase tracking-wider hover:bg-white hover:text-black transition-colors"
              >
                Clear Filters
              </button>
            </div>
      ...
      {destination.faq && destination.faq.length > 0 && (
        <section className="px-6 py-16 md:py-24 border-t border-white/[0.04]">
          <div className="max-w-[800px] mx-auto">
            <FAQAccordion items={destination.faq} title={`Questions about ${destination.name}`} />
          </div>
        </section>
      )}
        ...
        <section className="px-6 py-16 md:py-24 border-t border-white/[0.04]">
          <div className="max-w-[1200px] mx-auto">
            <div className="flex items-center justify-between mb-10">
              ...
              <Magnetic>
                <Link href="/destinations" className="px-5 py-2.5 rounded-full bg-white/[0.03] border border-white/[0.08] text-[10px] font-bold uppercase tracking-[0.15em] text-white/40 hover:bg-white hover:text-black transition-all duration-500">
                  View All
                </Link>
              </Magnetic>
            </div>
              ...
              <div className="flex gap-6" style={{ minWidth: 'max-content' }}>
                {relatedDestinations.map((dest) => (
                  <Link
                    ...
                    className="group flex-shrink-0 w-[300px] rounded-3xl overflow-hidden bg-white/[0.03] border border-white/[0.06] hover:border-white/[0.15] transition-all duration-500 hover:translate-y-[-4px]"
                  >
                    ...
                    <div className="flex items-center justify-between p-5 border-t border-white/[0.04]">
                      <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-white/30">{dest.region || "Explore"}</span>
